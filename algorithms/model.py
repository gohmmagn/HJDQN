...
class Critic_NN1(nn.Module):
    ...
    """
    def __init__(self, state_dim, action_dim, acctuator):
    ...
    def forward(self, state, action):
        acc = torch.matmul(action[:,[0]],torch.tensor(self.acctuator[0,:], dtype=torch.float).unsqueeze(0))
        for i in range(1,len(self.acctuator)):
          acc = acc + torch.matmul(action[:,[i]],torch.tensor(self.acctuator[i,:], dtype=torch.float).unsqueeze(0))        
...
class Critic_NN2(nn.Module):
    ...
    """
    def __init__(self, state_dim, action_dim, acctuator, resortIndex):
